...
for sub_index in range(1,1+sub_num):
    ...
    sort_index = np.arange(dataset.shape[0])
    ...
    for train_index, test_index in kf.split(dataset):
        ...
        model = PPG_MLP()
        # model = PPG_CNN()
        #model = FUS(rank = 8 , output_dim = 15, dropouts = [0.2, 0.2, 0.2, 0.2],fnn_output=15 ,use_softmax = True )
        #model = PPG_LSTM()
        ...
        for epoch_index in range(1,epochs+1):
            ...
            validate_y_pred = model(test_data, training  = False)
            ...
            if epoch_index == epochs:
                x = validation_sc_accuracy.result()
                x = x.numpy()
                statistic_score.append(x)

            print("第 {} 次， 训练集准确率：{}， 测试集准确率：{}".format(epoch_index, sparse_categorical_accuracy.result(),
                                                        validation_sc_accuracy.result()))

            validation_sc_accuracy.reset_state()
            sparse_categorical_accuracy.reset_state()

    print(statistic_score)
